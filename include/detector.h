...
//        cout << "copied"<<endl;
  ...
  //      make_image(image, data, x0, scale);
    ...
    void make_flattened_image(unsigned char *image, int *data, int *bg, float cof, float x0, float scale)
    {
    ...
    void make_flattened_image(unsigned char *image, int *data, float x0, float scale)
    {
        if(!image)cerr<<"make_image error: no image space";
        if(!data)cerr<<"make_image error: no data space";
        for(int i = 0; i < view_size; i++){
